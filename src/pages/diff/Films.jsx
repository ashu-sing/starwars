...
const Film = () => {
  ...
  const { data, status } = useQuery("films", fetchFilmsData, {
    onSuccess: () => console.log("Film data fetched"),
              ...
              {status === "loading" ? (
                ...
                Array.isArray(data?.results) ? (
                  data.results.map((film) => (
                    <li key={film.title} className="flex justify-between gap-x-6 py-5">
                      ...
                      <div className="hidden sm:flex sm:flex-col sm:items-end">
                        <p className="text-sm leading-6 text-slate-100">Release Date</p>
